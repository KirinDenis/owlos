...
Пример байт кода (проверяет значение освещенности и если освещенность выше заданной отключает освещение):
	...
	var _light=0                                  // опреление переменной _light и присвоение ей значения 0. Первый блок, он состоит всего из одной строки     
    getDeviceProp lightsensor, light, _light      //получить текущие показания сенсора с ID lightsensor. Начало второго блока. Эта строка считается "нулевой" для оператора перехода goto
    ifupper _light, 300, 4                        //если показани показания более 300 единиц выполнить инструкцию из строки 4
    setprop rele, data, 1                         //переходим сюда, если показания освещенности меньше или равны 300, замыкаем (включаем) реле с ID rele
    goto 0                                        //проверить показания с ID lightsensor снова
    setprop rele, data, 0                         //мы перешли сюда из 3 строки, если освещенность выше 300 ( _light > 300 ) - размыкаем (выключаем) реле
    goto 0                                        //проверить показания сенсора с ID lightsensor снова
