...
class TableWidget {
    ...
    constructor(devicesAnchors, devicesPanel, device, size) {
        ...
        //devices.addNewDeviceListner(this.newTable, this); //подписываемся на событие devices о создании нового устройства
    ...
    newTable() {
        ...
        */
        for (let devicePropertyKey of Object.keys(this.device)) {
    ...
    addProperty(deviceProperty) {
        ...
        //назначаем слушатель события изменения значения свойства, использующий созданную ссылку в качестве Sender 
        ...
        //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        deviceProperty.addValueListner(this.onDevicePropValueChangeGetaHref, getPropHref);

        //третъя колонка, текущее значение свойства устройства 
        ...
        //ссылка вызывающая API для назначения свойства устройства, так же как для воторой колонки 
        ...
        //так же как и для второй колонки - назначаем метод слушатель изменения объекта, он же "рисует" setPropHref элемент в 
        //зависимости от текущего значения свойства
        deviceProperty.addValueListner(this.onDevicePropValueChangeSetaHref, setPropHref);

        //четвертая колонка - редактор значения свойства, реализован так же как элементы для второй и третей колонки, со своим слушателем события
        ...
        deviceProperty.addValueListner(this.onDevicePropValueChangeEdit, edit);

        //пятая и шестая колонка, кнопки Set value и Get Value 
        ...
        if (!deviceProperty.type.includes("r")) {
    ...
    deviceLoaded(sender, device) {
    }


    //кноки подписываются на изменение сетевого статуса свойства устройства, этот метод слушатель соответствующего события в свойстве устройства
    ...
    onDevicePropNetworkStatusChange(sender, deviceProperty) {
        if (deviceProperty.networkStatus == NET_ONLINE) {
            ...
            if (deviceProperty.networkStatus == NET_RECONNECT) {
                ...
                if (deviceProperty.networkStatus == NET_OFFLINE) {
    ...
    onDevicePropValueChangeGetaHref(sender, deviceProperty) {
        ...
        if (deviceProperty.type.includes("s")) {
    ...
    onDevicePropValueChangeSetaHref(sender, deviceProperty) {
        ...
        if (deviceProperty.type.includes("b")) { //если тип свойства устройства boolean 
            if (deviceProperty.value === "1") sender.innerText = "true";
            else
                sender.innerText = "false";
        }
        else { //иначе текстовое представление - текст может быть очень длинным, ограничиваем в 20 символов
    ...
    onDevicePropValueChangeEdit(sender, deviceProperty) {
        if (deviceProperty.type.includes("b")) { //boolean
    ...
    setDeviceClick(event) {
        ...
        if (deviceProperty.networkStatus != NET_RECONNECT) {//если свойство устройства не в статуре "в реботе" - асинхронность это хорошо, но переполнять очередь это преступление
            ...
            if (edit.proptype.includes("b")) // boolean - представлен в виде combobox а не редактора 
            {
